...
def _render_filters(
    df: pd.DataFrame, people: list[str], all_tags: list[str]
) -> pd.DataFrame:
    with st.sidebar:
        st.header("Filters")
        ...
        date_range = st.date_input(
            "Date range", value=(min_date, max_date), key="analysis_date"
        )

        selected_people = st.multiselect(
            "Person", people, default=people, key="analysis_people"
        )

        available_tags = sorted(df["primary_tag"].unique().tolist())
        selected_tags = st.multiselect(
            "Tags", available_tags, default=available_tags, key="analysis_tags"
        )
...
def _render_charts(df: pd.DataFrame):
    ...
    fig.update_layout(
        yaxis_tickprefix="$", xaxis_title="", yaxis_title="Total Spending ($)"
    )
    ...
    with col_right:
        ...
        fig_bar.update_layout(
            yaxis_tickprefix="$", xaxis_title="", yaxis_title="Amount ($)"
        )
    ...
    fig_stacked = px.bar(
        monthly_tag, x="Month", y="Amount", color="Tag", barmode="stack"
    )
    ...
    with col_left2:
        ...
        fig_person = px.bar(
            by_person_month, x="Month", y="Amount", color="Person", barmode="group"
        )
    ...
    with col_right2:
        ...
        fig_dow = px.bar(
            by_dow, x="Day", y="Amount", color="Amount", color_continuous_scale="Blues"
        )
    ...
    fig_cum.update_layout(
        yaxis_tickprefix="$", xaxis_title="", yaxis_title="Cumulative Spending ($)"
    )
    st.plotly_chart(fig_cum, width="stretch")

    st.subheader("Top 10 Expenses")
    top10 = df.nlargest(10, "amount")[
        ["date", "description", "amount", "person", "tags_str"]
    ]
    top10 = top10.rename(
        columns={
            "date": "Date",
...
def _render_insights(df: pd.DataFrame):
    ...
    monthly_tag = df.groupby(["month", "primary_tag"])["amount"].sum().reset_index()
    avg_by_tag = (
        monthly_tag.groupby("primary_tag")["amount"]
        .mean()
        .sort_values(ascending=False)
        .reset_index()
    )
    avg_by_tag.columns = ["Tag", "Monthly Avg ($)"]
    avg_by_tag["Monthly Avg ($)"] = avg_by_tag["Monthly Avg ($)"].apply(
        lambda x: f"${x:,.2f}"
    )
    st.dataframe(avg_by_tag, width="stretch", hide_index=True)

    st.subheader("Spending Trend")
    monthly = df.groupby("month")["amount"].sum().reset_index()
    if len(monthly) >= 2:
        x = np.arange(len(monthly))
        y: np.ndarray = monthly["amount"].to_numpy(dtype=float)
        slope: float
        intercept: float
        r_value: float
        slope, intercept, r_value, _, __ = stats.linregress(x, y)  # type: ignore[assignment]
        direction = "up" if slope > 0 else "down"
        pct = abs(slope / (y.mean() or 1)) * 100
        st.info(
            f"Spending is trending **{direction}** by approximately **{pct:.1f}%** per month "
            f"(R²={r_value**2:.2f})."
        )
        ...
        fig_trend = go.Figure()
        fig_trend.add_trace(
            go.Scatter(x=monthly["month"], y=y, mode="lines+markers", name="Actual")
        )
    ...
    if anomalies:
        st.warning(
            f"{len(anomalies)} anomalous transaction(s) detected (>2 std above category mean)."
        )
    ...
    candidate_groups = df.groupby("description").filter(
        lambda g: len(g["month"].unique()) >= 2
    )
    if candidate_groups.empty:
        st.info(
            "No recurring expenses detected yet (need 2+ months of same description)."
        )
    else:
        recurring = (
    ...
    if len(months_sorted) >= 2:
        ...
        prior = (
            df[df["month"].isin(prior_months)].groupby("primary_tag")["amount"].sum()
        )
        recent = (
            df[df["month"].isin(recent_months)].groupby("primary_tag")["amount"].sum()
        )
        growth = pd.DataFrame({"Prior": prior, "Recent": recent}).fillna(0)
        growth["Change (%)"] = (
            (growth["Recent"] - growth["Prior"]) / (growth["Prior"].replace(0, np.nan))
        ) * 100
        ...
        growth["Change (%)"] = growth["Change (%)"].apply(
